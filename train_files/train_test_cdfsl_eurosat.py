...
for i, (data, label) in enumerate(d):
    # print(i, data)
    meta['image_names'].append(data)
    meta['image_labels'].append(label)
...
def compute_class_protos(imf, support_set_gt):
    n_classes = 5
    num_features = imf.shape[1]
    num_images = imf.shape[0]
    class_sums = torch.zeros((n_classes, num_features)).cuda()
...
def episode_loop():
    for idx, (x, y) in enumerate(novel_loader):
        ...
        support_set = support_set.reshape((support_set.shape[0] * support_set.shape[1]), 3, 64, 64).cuda() # [5,3,224,224] for 5 way-1 shot, [25, 3, 224, 224] for 5 way-5 shot
        query_set = query_set.reshape((query_set.shape[0] * query_set.shape[1]), 3, 64, 64).cuda() # [80, 3, 224, 224] for both settings
...
labels2classnames = {0: "AnnualCrop", 1: "Forest", 2: "HerbaceousVegetation", 3: "Highway", 4: "Industrial",
